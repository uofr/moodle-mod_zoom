...
$PAGE->set_url('/mod/zoom/report.php', array('id' => $id, 'mid' => $mid));
...
if (!($todisplay = $cache->get(strval($zoom->host_id))) || $ffrom != $todisplay->reqfrom || 
        $fto != $todisplay->reqto) {
    // Also send a new request if the from and to fields change from what we cached.
        ...
        foreach ($meetsessions as $starttime => $meet) {   
            ...
            if ($numparticipants > 0) {
                $url = new moodle_url('/mod/zoom/participants.php', 
                        array('id' => $id, 'mid' => $mid, 'session' => $starttime));
...
$dateform = new mod_zoom_report_form('report.php?id='.$id.'&mid='.$mid, array('id' => $id));
